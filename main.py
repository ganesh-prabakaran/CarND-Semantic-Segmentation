...
def load_vgg(sess, vgg_path):
    ...
    # Load saved model
    tf.saved_model.loader.loader.load(sess, [vgg_tag], vgg_path)
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    if TRANSFER_LEARNING_MODE:
        ...
        vgg_layer4_out = tf.stop_graident(vgg_layer4_out)
        vgg_layer3_out = tf.stop_gradient(vgg_layer3_out)
    
    # Upsample
    vgg_layer7_out_2x = tf.layers.conv2d_transpose(vgg_layer7_out, num_classes, 4, strides = (2,2), padding = "SAME",\
    kernel_regularization = tf.contrib.layers.l2_regularizer(1e-3), activation = 'RELU', name = "DECON_vgg_layer7_out_2x")
    
    vgg_layer4_7_combined = tf.add(vgg_layer7_out_2x, vgg_layer4_out, name = "DECON_vgg_layer4_7_combined")
    
    vgg_layer4_out_4x = tf.layers.conv2d_transpose(vgg_layer4_7_combined, num_classes, 4, strides = (2,2), padding = "SAME",\
    kernel_regularization = tf.contrib.layers.l2_regularizer(1e-3), activation = 'RELU', name = "DECON_vgg_layer7_out_4x")
    
    vgg_layer3_7_combined = tf.add(vgg_layer7_out_4x, vgg_layer3_out, name = "DECON_vgg_layer3_7_combined")
    
    nn_last_layer = tf.layers.conv2d_transpose(vgg_layer3_7_combined, num_classes, 16, strides = (8,8), padding = "SAME",\
    kernel_regularization = tf.contrib.layers.l2_regularizer(1e-3), activation = 'RELU', name = "DECON_nn_last_layer")
    
	
    return vgg_layer7_out_8x
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
	...
	for variable in tf.trainable_variables():
		if "DECON_" in variable.name or 'beta' in variable.name:
    ...
    return logits, training_op, loss_operation
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    # TODO: Implement function
	if TRANSFER_LEARNING_MODE:
		variable_initializers = [ var.initializer for var in tf.global_variables() if 'DECON_' in var.name or 'beta' in var.name ]
